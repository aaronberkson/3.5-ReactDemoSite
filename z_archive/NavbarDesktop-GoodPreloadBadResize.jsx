...
const LINE_2_MIT_MAIN = (
  <>
    <strong className="glow-mit">MIT xPRO</strong>&nbsp;MERN Certificate
  </>
);
  ...
  useLayoutEffect(() => {
    const recalcLine2 = () => {
      if (textContainerRef.current) {
        ...
        const safeAvailableWidth = availableWidth - safetyMargin;
        const text = typeof secondLine.main === "string" ? secondLine.main : "";
        const temp = document.createElement("span");
        temp.style.fontSize = DEFAULT_LINE_2 + "px";
        temp.style.fontFamily = "var(--font-focal-medium)";
        temp.style.whiteSpace = "nowrap";
        temp.style.visibility = "hidden";
        temp.innerText = text;
        document.body.appendChild(temp);
