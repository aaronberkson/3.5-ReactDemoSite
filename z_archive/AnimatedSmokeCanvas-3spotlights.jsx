...
const SPOTLIGHT_DISTANCE_MULTIPLIER = 2.0;  // Increase to let the light travel further.
const SPOTLIGHT_ANGLE_MULTIPLIER = 1.5;       // Increase to widen the beam.

// ─────────────────────────────────────────────────────────────
// Spotlight Parameters – using intuitive positions.
// 
// LEFT SPOTLIGHT: 
//   Positioned by its pixel offset from the left and top.  
//   We set a lower Y offset (50px from the top) so its effect appears near the top,
//   and we aim it down and inward by setting its direction from 315° (which is equivalent to –45°).
const SPOTLIGHT_LEFT_OFFSET = new THREE.Vector2(25, 100);   // 100px from left, 50px from top.
...
const SPOTLIGHT_LEFT_ANGLE_DEG = -45.0; // Conceptually -45 degrees.
const SPOTLIGHT_LEFT_DIR = new THREE.Vector2(
  Math.cos(THREE.MathUtils.degToRad(315)), // 315° === -45°
  Math.sin(THREE.MathUtils.degToRad(315))
);
const SPOTLIGHT_LEFT_ANGLE_INNER = THREE.MathUtils.degToRad(0.0);
const SPOTLIGHT_LEFT_ANGLE_OUTER = THREE.MathUtils.degToRad(20.0 * SPOTLIGHT_ANGLE_MULTIPLIER);

// RIGHT SPOTLIGHT:
//   Positioned by its pixel offset from the right and top.
//   Its direction is set from 225° (i.e. -135°), which points down and inward.
const SPOTLIGHT_RIGHT_OFFSET = new THREE.Vector2(25, 100);  // 150px from right, 50px from top.
...
const SPOTLIGHT_RIGHT_ANGLE_DEG = -135.0; // Conceptually -135 degrees.
const SPOTLIGHT_RIGHT_DIR = new THREE.Vector2(
  Math.cos(THREE.MathUtils.degToRad(225)), // 225° 
  Math.sin(THREE.MathUtils.degToRad(225))
);
const SPOTLIGHT_RIGHT_ANGLE_INNER = THREE.MathUtils.degToRad(0.0);
const SPOTLIGHT_RIGHT_ANGLE_OUTER = THREE.MathUtils.degToRad(20.0 * SPOTLIGHT_ANGLE_MULTIPLIER);

// MIDDLE SPOTLIGHT:
//   Always centered horizontally (x = 0.5) with a vertical offset from the top.
...
const SPOTLIGHT_MIDDLE_DIR = new THREE.Vector2(0.0, -1.0); // Points straight down.
